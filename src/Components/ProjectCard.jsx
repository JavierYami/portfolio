...
const ProjectCard = ({name, description, image, deploy, repository}) => {
    ...
    return (
        <article className="grid grid-cols-2 m-20 p-5">
          <div className="w-5/6">
            <h1 className="text-cyan-500 text-3xl font-bold text-center mb-5">{name}</h1>
            <p className="text-justify">{description}</p>
          </div>
          <div className="rounded-lg border-cyan-500 border-dashed border-4 p-1 w-6/6 relative">
            <div className="absolute inset-0 flex flex-col items-center justify-center space-y-5 opacity-0 hover:opacity-100 transition-opacity">
                <a href={deploy} target="_blank" className="bg-cyan-500 text-white px-4 py-1 rounded transition duration-300 ease-in-out w-2/6 text-center truncate hover:bg-indigo-500">{`Visit ${name}`} <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3 inline-block ml-1">
